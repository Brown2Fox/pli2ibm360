...
#define NPOP 7 /*размер табл.псевдоопер. */
...
/*..........................................................................*/
/*п р о т о т и п  обращ.к*/
int FEXTRN(); /*подпр.обр.пс.опер.EXTRN */
/*..........................................................................*/
/*п р о т о т и п  обращ.к*/
...
/*..........................................................................*/
/*п р о т о т и п  обращ.к*/
int SEXTRN(); /*подпр.обр.пс.опер.EXTRN */
/*..........................................................................*/
/*п р о т о т и п  обращ.к*/
...
struct STR_BUF_ESD /*структ.буфера карты ESD */
{
    ...
    unsigned char CARD_TYPE[3]; /*поле типа об'ектн.карты */
    ...
    unsigned char ESD_TYPE; /*код типа ESD-имени      */
...
struct STR_BUF_RLD /*структ.буфера карты RLD */
{
    unsigned char PADDING1; /*место для кода 0x02     */
    unsigned char CARD_TYPE[3]; /*поле типа об'ектн.карты */
    unsigned char PADDING2; /*пробелы                 */
    unsigned char ID_NUM[2]; /*внутр.ид-р                 */
    unsigned char PADDING3[3]; /*пробелы                 */
    unsigned char SIGN[2]; /* знак                 */
    unsigned char PADDING4[4]; /*пробелы                 */
    unsigned char OFFSET_ADDR[3]; /*относит.адрес опреации  */
    unsigned char PADDING5[53]; /*пробелы                 */
...
union /*определить об'единение  */ 
{
    struct STR_BUF_RLD STR_RLD; /*структура буфера        */
    unsigned char BUF_RLD[80]; /*буфер карты RLD         */
} RLD;
...
void alignAddr(unsigned* addr, unsigned by)
{
    if (*addr % by != 0)
    {
    ...
    { /*то:                     */
        if /* если псевдооперация DC */
            (/* определяет константу   */
                ...
                || TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND[0] == 'A') /* следующее:             */
        {
...
int RLD_ID_NUM = 1;
int ESD_ID_NUM = 0;
void SRLD() /*подпр.формир.RLD-карты  */
{
    char* PTR; /*рабоч.переменная-указат.*/

    PTR = (char*)&CHADR; /*формирование поля ADOP  */
    RLD.STR_RLD.OFFSET_ADDR[2] = *PTR; /*TXT-карты в формате     */
    RLD.STR_RLD.OFFSET_ADDR[1] = *(PTR + 1); /*двоичного целого        */
    RLD.STR_RLD.OFFSET_ADDR[0] = '\x00'; /*в соглашениях ЕС ЭВМ    */

    RLD.STR_RLD.SIGN[0] = 0x0;
    RLD.STR_RLD.SIGN[1] = 0xC;

    RLD.STR_RLD.ID_NUM[0] = (unsigned char)RLD_ID_NUM >> 8;
    RLD.STR_RLD.ID_NUM[1] = (unsigned char)RLD_ID_NUM;
    RLD_ID_NUM++;

    memcpy(OBJTEXT[ITCARD], RLD.BUF_RLD, 80);
    ITCARD += 1;
}
...
int SDC() /*подпр.обр.пс.опер.DC    */
{
    ...
    else if (!memcmp(TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND, "A(", 2))
    {
...
int SSTART() /*подпр.обр.пс.опер.START */
{
    ...
    { /* табл. T_SYM сравниваем */
        ...
        { /*  берем разность сч.адр.*/
            ...
            ESD.STR_ESD.ID_NUM[0] = (unsigned char)ESD_ID_NUM >> 8;
            ESD.STR_ESD.ID_NUM[1] = (unsigned char)ESD_ID_NUM;
            ESD_ID_NUM++;
            memcpy(/*запись об'ектной карты  */
                OBJTEXT[ITCARD], /* в                      */
                ESD.BUF_ESD, /* массив                 */
...
int SUSING() /*подпр.обр.пс.опер.USING */
{
    ...
    if (isIdentifier(METKA2[0])) /*если лексема начинается */
    { /*с буквы, то:            */
...
/*..........................................................................*/
int SEXTRN() /*подпр.обр.пс.опер.USING */
{
    char* sym_name_asm = strtok((char*)TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND, " ");


    ESD.STR_ESD.DLPRG[0] = 0;
    ESD.STR_ESD.DLPRG[1] = 0;
    ESD.STR_ESD.DLPRG[2] = 0;

    ESD.STR_ESD.ADPRG[0] = 0;
    ESD.STR_ESD.ADPRG[1] = 0;
    ESD.STR_ESD.ADPRG[2] = 0;

    ESD.STR_ESD.ESD_TYPE = 2;

    memcpy(ESD.STR_ESD.IMPR, sym_name_asm, strlen(sym_name_asm));
...
int SRR() /*подпр.обр.опер.RR-форм. */
{
    ...
    if (isIdentifier(METKA1[0])) /*если лексема начинается */
    { /*с буквы, то:            */
        for (J = 0; J <= ITSYM; J++) /* все метки исх.текста в */
        { /* табл. T_SYM сравниваем */
            /* со знач.перем. *METKA1 */
...
void INITUNION()
{
    ...
    */
    ...
    memcpy(ESD.STR_ESD.CARD_TYPE, "ESD", 3);
    memset(ESD.STR_ESD.POLE3, 0x40, 6);
    ESD.STR_ESD.POLE31[0] = 0x40;
    ESD.STR_ESD.POLE31[1] = 0x40;
    memset(ESD.STR_ESD.POLE32, 0x40, 2);
    ESD.STR_ESD.ID_NUM[0] = 0x00;
    ESD.STR_ESD.ID_NUM[1] = 0x00;
    memset(ESD.STR_ESD.IMPR, 0x40, 8);
    ESD.STR_ESD.ESD_TYPE = 0x00;
    ...
    */
    ...
    /*
    ***** и н и ц и а л и з а ц и я   полей буфера формирования записей RLD-типа
    *****                             для выходного объектного файла
    */

    RLD.STR_RLD.PADDING1 = 0x02;
    memcpy(RLD.STR_RLD.CARD_TYPE, "RLD", 3);
    RLD.STR_RLD.PADDING2 = 0x40;
    memset(RLD.STR_RLD.ID_NUM, 0x40, 2);
    memset(RLD.STR_RLD.PADDING3, 0x40, 3);
    memset(RLD.STR_RLD.SIGN, 0x40, 2);
    memset(RLD.STR_RLD.PADDING4, 0x40, 4);
    memset(RLD.STR_RLD.OFFSET_ADDR, 0x40, 3);
    memset(RLD.STR_RLD.PADDING5, 0x40, 53);
    memset(RLD.STR_RLD.ID_FIELD, 0x40, 8);
}
...
int main(int argc, char** argv) /*главная программа       */
{
    ...
    */
    ...
    fclose(fp);
    NFIL[strlen(NFIL) - 3] = '\0';

    for (int i = 0; i < DL_ASSTEXT; i++)
    {
        printf("%i | %.80s|\n", i+1, ASSTEXT[i]);
    }
...
CONT6:
    printf("%s%d\n", "ошибка в строке N ", I1 + 1); /*выдать диагностич.сообщ.*/
