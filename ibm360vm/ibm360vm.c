...
/*машинных                */
/*операций                */
/*об'явление табл.маш.опер*/
        T_MOP[NOP] = {{{'B', 'A', 'L', 'R', ' '}, '\x05', 2, FRR},
                      {{'B', 'C', 'R', ' ', ' '}, '\x07', 2, FRR},
                      {{'S', 'T', ' ', ' ', ' '}, '\x50', 4, FRX},
                      {{'L', ' ', ' ', ' ', ' '}, '\x58', 4, FRX},
                      {{'A', ' ', ' ', ' ', ' '}, '\x5A', 4, FRX},
...
// программа реализации семантики команды BALR
int P_BALR(void)
{
    if (R2 != 0)
        I = VR[R2];
...
// программа реализации семантики команды BCR с маской 15
int P_BCR(void)
{
    int ret;
    ...
    if (R1 == 15)
    {
        ...
        else
        {
            if (R2 != 0)
            {
                waddstr(wcyan, "go to address 0 or completion of the program trace "
...
/* программа реализации семантики команды ST */
int P_ST()
{
    /*рабочие                 */
    /*переменные              */
    ...
    /*                        */
    /*запись преобразованого  */
    /*значения по адресу 2-г  */
    /*операнда                */
    for (i = 0; i < 4; i++)
        OBLZ[BAS_IND + CUR_IND + sm + i] = bytes[i];

    /*успешное заверш.прогр.  */
    return 0;
}

/*  программа ареализации семантики команды L */
int P_L()
{
...
/* программа реализации семантики команды А */
int P_A()
{
    int sm; /*рабочая переменная      */
...
int sys(void)
{
    ...
    waddstr(wmargenta, "PgUp,PgDn,Up,Down->viewing a dump; Enter->execute next command");
...
l0:
    ...
    if (gr_pos_y > 14 - gr_y + 1)
        ...
        //при достижении некоторого положения, движение останавливается, и
        //производится
        //прокрутка окна
    else
    {
    ...
    waddstr(wcyan, "Readiness to execute the next command with the address ");
...
ERR1:
    printf("File open error with list of building modules\n");
    goto END;

ERR2:
    printf("Empty file with list of building modules\n");
    goto END;
...
ERR4:
    printf("Building modules list overflow error\n");
    goto END;

ERR5:
    printf("Object maps storage buffer overflow error\n");
    goto END;
...
ERR9:
    printf("Invalid file type with source code\n");
    goto END;
}